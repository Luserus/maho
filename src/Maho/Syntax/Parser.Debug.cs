...
internal sealed partial class Parser
{
    ...
    private string FormatNode(SyntaxNode node, Dictionary<SyntaxNode, TextSpan?> spanCache)
    {
        ...
        var spanText = span is TextSpan value ? $" {Colorize(FormatSpan(value), Dim)}" : string.Empty;

        if (node is Token token)
        {
            var matchingKind = token.MatchingKind is MatchingKeywordKind.None
                ? string.Empty
    ...
    private static bool ShouldUseColor()
    {
        ...
        return !string.IsNullOrEmpty(term) && !string.Equals(term, "dumb", System.StringComparison.OrdinalIgnoreCase);
    }
}
